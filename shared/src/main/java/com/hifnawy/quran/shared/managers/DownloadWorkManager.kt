...
class DownloadWorkManager(private val context: Context, workerParams: WorkerParameters) :
    ...
    private suspend fun downloadChunk(
        ...
        (withContext(Dispatchers.IO) { url.openConnection() } as HttpURLConnection).apply {
            ...
            var bytesDownloaded = chunk.startOffset
            ...
            do {
                ...
                mutex.withLock {
                    ...
                    // Log.d(
                    //         TAG,
                    //         "[$timestamp] $chunkName - read ${bytes.formatted} bytes, total: ${(bytesDownloaded + bytes).formatted} bytes!"
                    // )
                    chapterFileStream.seek(bytesDownloaded)
                    chapterFileStream.write(buffer, 0, bytes)

                    bytesDownloaded += bytes

                    chunk.currentOffset = bytesDownloaded
            ...
            } while (!isStopped && (bytesDownloaded < chunk.endOffset))

            Log.d(
                    TAG,
                    "[$timestamp] $chunkName: ${
                        if (isStopped) "DOWNLOAD JOB INTERRUPTED at offset ${bytesDownloaded.formatted}!!!"
    ...
    private fun postDownloadCleanup(
        ...
        if (!downloadInterrupted) {
            ...
            for (chunk in chunks) {
                val buffer = ByteArray(chunk.downloaded.toInt())
