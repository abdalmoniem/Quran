...
class SharedPreferencesManager(private val context: Context) {
    private enum class SharedPrefsKeys {
        RECITERS, CHAPTERS, LAST_RECITER, LAST_CHAPTER, LAST_CHAPTER_POSITION, LAST_CHAPTER_DURATION, RECITER_CHAPTER_AUDIO_FILES, MEDIA_PATH_CONSISTENCY
    }
    ...
    var reciters: List<Reciter>?
        ...
        set(value) = sharedPrefs.edit().putSerializableList(SharedPrefsKeys.RECITERS.name, value!!)
            .apply()
    var chapters: List<Chapter>?
        get() = sharedPrefs.getSerializableList(SharedPrefsKeys.CHAPTERS.name)
        set(value) = sharedPrefs.edit().putSerializableList(SharedPrefsKeys.CHAPTERS.name, value!!)
            .apply()
    ...
    var lastChapterDuration: Long
        ...
        set(value) = sharedPrefs.edit().putLong(SharedPrefsKeys.LAST_CHAPTER_DURATION.name, value)
            .apply()
    ...
    fun setReciterChapterAudioFiles(reciterID: Int, chapterAudioFiles: List<ChapterAudioFile>) =
        sharedPrefs.edit()
            .putSerializableList("${SharedPrefsKeys.RECITER_CHAPTER_AUDIO_FILES.name}_#$reciterID", chapterAudioFiles)
            .apply()
